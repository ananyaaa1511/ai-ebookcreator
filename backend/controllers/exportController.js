...
const exportAsDocument = async (req, res) => {
    try {
        ...
        // Fix: Absolute path resolution for Word Document export
        if (book.coverImage && !book.coverImage.includes("pravatar")) {
            ...
            try {
                ...
                console.error(`Could not embed Word image: ${imagePath}`, imgErr);
            }
        }
...
const exportAsPDF = async (req, res) => {
    try {
        ...
        // Fix: Use correct absolute local disk file path resolution for PDFKit
        if (book.coverImage && !book.coverImage.includes("pravatar")) {
            const imagePath = path.join(__dirname, "..", book.coverImage);
            ...
            try {
                if (fs.existsSync(imagePath)) {
                    ...
                    console.error(`PDF Image file does not exist at path: ${imagePath}`);
                }
            } catch (imgErr) {
                console.error(`Could not embed PDF image: ${imagePath}`, imgErr);
            }
        }
